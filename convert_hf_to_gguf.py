...
class ModelBase:
    ...
    def tensor_force_quant(self, name: str, new_name: str, bid: int | None, n_dims: int) -> gguf.GGMLQuantizationType | bool:
        # Force F32 for TTS-specific tensors (emb_code, emb_text, projector_semantic, head_code)
        # These need full precision for accurate TTS condition generation and audio token prediction
        if (new_name.startswith("emb_code.") or
            new_name.startswith("emb_text.") or 
            new_name.startswith("projector_semantic.") or
            new_name.startswith("head_code.")):
    ...
    def prepare_tensors(self):
        ...
        # First pass: collect head_code parametrizations from raw tensors (before modify_tensors)
        # We need to access raw tensors to get parametrizations before they're filtered out
        head_code_parametrizations = {}
        raw_tensors = list(chain(self.generate_extra_tensors(), self.get_tensors()))
        
        for name, data_torch in raw_tensors:
            if "head_code" in name and "parametrizations" in name:
                # Convert lazy tensor to eager if needed
                if isinstance(data_torch, gguf.LazyBase):
                    data_torch = LazyTorchTensor.to_eager(data_torch)
                head_code_parametrizations[name] = data_torch
        
        # Reconstruct head_code weights from parametrizations
        # weight_norm: W = g * v / ||v||
        # where g is original0 (scale, shape=[6562, 1]) and v is original1 (direction, shape=[6562, 768])
        reconstructed_head_code = None
        for name, data_torch in raw_tensors:
            if "head_code.0.parametrizations.weight.original1" in name:
                # Found the direction vector v, now find the scale g
                scale_name = name.replace("original1", "original0")
                if scale_name not in head_code_parametrizations:
                    logger.warning(f"Could not find scale parameter for head_code: {scale_name}")
                    logger.warning(f"Available parametrizations: {list(head_code_parametrizations.keys())}")
                    continue
                
                g = head_code_parametrizations[scale_name]  # [6562, 1]
                v = data_torch  # [6562, 768]
                
                # Convert lazy tensors to eager if needed
                if isinstance(g, gguf.LazyBase):
                    g = LazyTorchTensor.to_eager(g)
                if isinstance(v, gguf.LazyBase):
                    v = LazyTorchTensor.to_eager(v)
                
                # Verify shapes
                if len(g.shape) != 2 or g.shape[0] != 6562 or g.shape[1] != 1:
                    logger.error(f"head_code scale parameter has wrong shape: expected [6562, 1], got {g.shape}")
                    continue
                if len(v.shape) != 2 or v.shape[0] != 6562 or v.shape[1] != 768:
                    logger.error(f"head_code direction parameter has wrong shape: expected [6562, 768], got {v.shape}")
                    continue
                
                # Compute L2 norm of v along the last dimension: ||v||
                v_norm = torch.norm(v, p=2, dim=1, keepdim=True)  # [6562, 1]
                # Avoid division by zero
                v_norm = torch.clamp(v_norm, min=1e-8)
                
                # Reconstruct original weight: W = g * v / ||v||
                # W shape: [6562, 768] (PyTorch format: out_features, in_features)
                W = g * v / v_norm
                
                # Verify reconstructed weight shape
                if W.shape != (6562, 768):
                    logger.error(f"Reconstructed head_code has wrong shape: expected [6562, 768], got {W.shape}")
                    continue
                
                # Transpose to match C++ expected format: (hidden_size, num_audio_tokens) = [768, 6562]
                # C++ code expects: hidden_state @ head_code_weight
                # where hidden_state is (768,) and head_code_weight is (768, 6562)
                W_transposed = W.T  # Shape: [768, 6562]
                
                if W_transposed.shape != (768, 6562):
                    logger.error(f"Transposed head_code has wrong shape: expected [768, 6562], got {W_transposed.shape}")
                    continue
                
                reconstructed_head_code = ("head_code.0.weight", W_transposed)
                logger.info(f"Reconstructed {reconstructed_head_code[0]} from weight_norm parametrizations: original_shape={W.shape}, transposed_shape={W_transposed.shape}")
                break
        
        # Warn if head_code was not reconstructed
        if reconstructed_head_code is None:
            logger.error("Failed to reconstruct head_code.0.weight from parametrizations!")
            logger.error("This will cause TTS audio generation to fail!")
            logger.error(f"Available parametrizations: {list(head_code_parametrizations.keys())}")

        # Now process all tensors through modify_tensors
        all_tensors = []
        for name, data_torch in raw_tensors:
            # Skip head_code parametrizations - they've been reconstructed
            if "head_code" in name and "parametrizations" in name:
                continue
            
            # Extract block id from tensor name (for modify_tensors)
            bid = None
            ...
            # Process through modify_tensors
            for new_name, modified_tensor in self.modify_tensors(data_torch, name, bid):
                all_tensors.append((new_name, modified_tensor))
        
        # Add reconstructed head_code weight
        if reconstructed_head_code:
            all_tensors.append(reconstructed_head_code)
            logger.info(f"Added reconstructed head_code.0.weight to tensor list")
        else:
            logger.error("head_code.0.weight was NOT reconstructed and will be missing from GGUF file!")
            logger.error("TTS audio generation will fail without head_code.0.weight!")
            logger.error(f"Available parametrizations: {list(head_code_parametrizations.keys())}")

        for name, data_torch in all_tensors:
            # Skip parametrizations - they've been reconstructed into head_code.0.weight
            ...
            for new_name, data_torch in (self.modify_tensors(data_torch, name, bid)):
                ...
                # Also force F32 for TTS-specific tensors (emb_text, projector_semantic, head_code)
                if (n_dims <= 1 or 
                    new_name.endswith("_norm.weight") or
                    new_name.startswith("emb_text.") or
                    new_name.startswith("projector_semantic.") or
                    new_name.startswith("head_code.")):
...
class LlamaModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # Skip TTS-specific tensors (BUT keep emb_code, emb_text, projector_semantic, head_code)
        # Note: projector_spk is also kept in case it's needed in the future
        # Note: head_code parametrizations are kept temporarily to reconstruct original weight in prepare_tensors
        is_tts_tensor_to_skip = ("parametrizations" in name and "head_code" not in name)
        
        # Keep emb_code, emb_text, projector layers, and head_code - these are needed for TTS:
        # emb_code: for converting audio token IDs to embeddings (used in decode phase)
        # emb_text: for converting token IDs to embeddings (used in prefill phase)
        # projector_semantic: for projecting LLM hidden states to TTS hidden dim
        # head_code: for generating audio token logits from TTS hidden states
        # projector_spk: for speaker embedding (kept for completeness, may not be used)
        is_tts_tensor_to_keep = name.startswith("emb_code.") \
            or name.startswith("emb_text.") \
            or name.startswith("projector_semantic.") \
            or name.startswith("projector_spk.") \
            or name.startswith("head_code.")

        if is_multimodal_tensor or is_tts_tensor_to_skip:
            return [] # skip vision/tts tensors that are not needed
        
        # Handle TTS tensors that should be kept (emb_text, projector_semantic)
        # Note: head_code parametrizations will be handled in prepare_tensors to reconstruct original weight
        if is_tts_tensor_to_keep:
            # Keep the original name for TTS-specific tensors
            # These will be added to GGUF with their original names (no mapping needed)
            # Use original name directly since these are custom TTS tensors
            return [(name, data_torch)]
        
        # Handle TTS model's embed_tokens.weight: extend it to match tokenizer vocab_size
        # TTS models use LlamaModel as backbone, which creates embed_tokens with default vocab_size (~32000)
        # But the tokenizer has vocab_size=151748. We need to extend embed_tokens to match tokenizer.
        # Note: TTS models actually use emb_text (152064) instead of embed_tokens, but llama.cpp
        # still checks embed_tokens.shape during model loading.
        if "embed_tokens.weight" in name:
            # Get tokenizer vocab size from config (should match tokenizer vocab_size)
            tokenizer_vocab_size = self.hparams.get("vocab_size", 151748)
            
            # Check if embed_tokens needs to be extended
            current_vocab_size = data_torch.shape[0]
            hidden_size = data_torch.shape[1]
            
            if current_vocab_size < tokenizer_vocab_size:
                logger.info(f"TTS model detected: extending embed_tokens.weight from {current_vocab_size} to {tokenizer_vocab_size} to match tokenizer vocab_size")
                logger.info(f"  Original shape: {data_torch.shape}, dtype: {data_torch.dtype}, device: {data_torch.device}")
                # Ensure data_torch is on CPU (in case it's on meta device)
                if data_torch.device.type == 'meta':
                    # If on meta device, we need to materialize it first
                    # This shouldn't happen in normal conversion, but handle it just in case
                    logger.warning(f"  Warning: data_torch is on meta device, this may cause issues")
                # Move to CPU if needed for concatenation
                data_torch_cpu = data_torch.cpu() if data_torch.device.type != 'cpu' else data_torch
                # Extend embed_tokens.weight with zero padding
                pad_size = tokenizer_vocab_size - current_vocab_size
                # Create zero padding on CPU: (pad_size, hidden_size)
                padding = torch.zeros((pad_size, hidden_size), dtype=data_torch_cpu.dtype, device='cpu')
                data_torch = torch.cat([data_torch_cpu, padding], dim=0)
                logger.info(f"  Extended shape: {data_torch.shape}, dtype: {data_torch.dtype}, device: {data_torch.device}")
                logger.info(f"Extended embed_tokens.weight: {current_vocab_size} -> {tokenizer_vocab_size}")
            elif current_vocab_size > tokenizer_vocab_size:
                logger.warning(f"embed_tokens.weight vocab_size ({current_vocab_size}) > tokenizer vocab_size ({tokenizer_vocab_size}), this may cause issues")
